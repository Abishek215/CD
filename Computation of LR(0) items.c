...
// 3. Open the File and Read the input from it 
// 4. Build state 0 from extra grammar Law S' -> S $ that is all start symbol of grammar and one Dot ( . ) before S symbol. 
// 5. If the Dot symbol is before a non-terminal, add grammar laws that this non-terminal is on Left Hand Side of that Law and set Dot in before of first part of Right Hand Side 
// 6. If a state exists (a state with these Laws and the same Dot position), use that instead. 
// 7. Now find the set of terminals and non-terminals in which Dot exists before
//  8. If Step 7 Set is non -empty go to Step 9 else go to Step 10 
// 9. For each terminal/non-terminal in the set of step 7 create a new state by using all grammar law that Dot position is before of that terminal/non-terminal in reference state by increasing Dot point to next part in Right Hand Side of that laws. 
// 10.Go to Step 5 
