// Algorithm:
// The algorithm takes a sequence of three- address statements as input. For each three address statements of the form a:=b on c perform the various actions. These are as follows:

// 1.	Invoke a function getreg to find out the location L where the result of computation b or c should be stored.
...
// 4.	If the current value of y or z have no next uses or not live on exit from the block or in the register then alter the register descriptor to indicate that after execution of x: = y op z those register will no longer contain y or z
